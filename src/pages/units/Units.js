...
const Units = () => {
    ...
    useEffect(() => {
        ...
        const url = new URL(`${API}/api/v1/unit`);
    ...
    return (
        <div className="categories-container">
            ...
            <div className="options">
                ...
                <Link to="/new-unit" className="add-box">
                    <FontAwesomeIcon icon={faPlus} className="icon" />
                    <span className="text">New Shop</span>
                </Link>
            </div>
            ...
            {!isLoading ? (
                <div className="table-container">
                    <table className="table">
                        ...
                        <tbody>
                            {paginator.unitDTOS && paginator.unitDTOS.length > 0 ? (
                                paginator.unitDTOS.map(unit => (
                                    <tr key={unit.unitId}>
                                        {/* <td>{unit.unitId}</td> */}
                                        <td>{unit.name}</td>
                                        <td>{unit.phone}</td>
                                        <td>
                                            <Link to={`/edit-unit/${unit.unitId}`}>
