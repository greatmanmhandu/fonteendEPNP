...
const EditPreTerm = () => {
    ...
    useEffect(() => {
        ...
        // Query data
        (async () => {
            const url = new URL(`${API}/api/v1/preterm/${id}`);
            await fetch(url)
                .then(response => response.json())
                .then(data => {
                    if (!data || data.error) {
                        navigate('/pre-term-records');
                        return;
                    }
                    setFormData({
                        name: data.name,
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch(`${API}/api/v1/preterm/${id}`, {
                method: 'PUT',
                headers: {
    ...
    return (
        <div className="editItem-container">
            ...
            <div className="text">Edit Expense</div>
            {!isLoading ? (
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="grid-form">
                            ...
                            <div className="two-together">
                                ...
                                <div className="form-item">
                                    <label htmlFor="weight">Weight (Unit of measuare)</label>
                                    <input
                                        className="input"
                                        type="text"
                                        id="weight"
