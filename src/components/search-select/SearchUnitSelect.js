...
const SearchUnitSelect = ({ label, placeholder, onSelected, apiUrl, optionsAttr, initialSelectedOption = null, isRequired = false }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.log(data[0].unitList)
                const unitList = data[0].unitList || [];
                setOptions(unitList);
                
                if (initialSelectedOption && !selectedOption) {
                    const selected = unitList.find(unit => getIdKey(unit) === getIdKey(initialSelectedOption));
                    if (selected) {
        ...
        fetchData();
    }, [apiUrl, optionsAttr, initialSelectedOption, selectedOption, onSelected]);
    ...
    const getIdKey = (option) => {
        const idKey = Object.keys(option).find(key => key.toLowerCase().includes('id'));
        return option[idKey];
    }
                            ...
                            options.map((option) => (
                                ...
                                >
                                    {({ selectedOption, active }) => (
                                        <>
                                            <span className={`block truncate ${selectedOption ? 'font-medium' : 'font-normal'}`}>
                                                {option.name}
                                            </span>
                                            {selectedOption ? (
...
export default SearchUnitSelect;